...
namespace :deploy do
  ...
  task :save_members_technologies do
    on roles(:web) do
      ...
      execute "cd #{current_path}; rake db:save_members_technologies RAILS_ENV=#{rails_env}"
    end
  end
  ...
  task :save_projects_technologies do
    on roles(:web) do
      rails_env = fetch(:rails_env, 'staging')
      execute "cd #{current_path}; rake db:save_projects_technologies RAILS_ENV=#{rails_env}"
    end
  end
  ...
  task :load_members_technologies do
    on roles(:web) do
      rails_env = fetch(:rails_env, 'staging')
      execute "cd #{current_path}; rake db:load_members_technologies RAILS_ENV=#{rails_env}"
    end
  end
  ...
  task :load_projects_technologies do
    on roles(:web) do
      rails_env = fetch(:rails_env, 'staging')
      execute "cd #{current_path}; rake db:load_projects_technologies RAILS_ENV=#{rails_env}"
    end
  end
